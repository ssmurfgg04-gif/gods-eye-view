 ...
 * @description OpenAI Realtime session tool schemas for the GEV voice agent.
 *
 * Extracted verbatim from vite.config.js (620 lines of pure tool-schema
 * data) so the proxy config file composes plugins instead of embedding an
 * LLM tool catalog. The ONLY consumers are the realtime token/session
 * endpoints (server) and the tool-schema validation smoke test — the data
 * is deliberately browser-free (no Cesium, no DOM).
 */
