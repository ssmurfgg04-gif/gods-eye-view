...
| **NIFC WFIGS** | Wildfire incidents (human-reported, WF/CX only) | U.S. public domain incident tracking | "NIFC WFIGS (public incident tracking)" |
| **OpenSeaMap** | Sea-marks raster overlay | ODbL 1.0 | "© OpenSeaMap contributors" |
| **OpenSnowMap** | Ski-pistes raster overlay | CC-BY-SA (style), ODbL/SRTM (data) | "Map style © OpenSnowMap.org (CC-BY-SA)" |
...
| **GTFS-Realtime vehicle-position feeds** — MBTA (Boston), CapMetro (Austin), Metro Transit (Minneapolis–St Paul), HSL (Helsinki), OVapi (Netherlands), Entur (Norway), TransLink (South East Queensland) | The Transit layer's live vehicles, via `/api/transit` | Per feed, all keyless (MassDOT terms · Texas Open Data terms · Minnesota public domain · CC BY 4.0 HSL · CC0 OVapi · NLOD Entur · CC BY 4.0 TransLink — see note) | Each operator credited in the Data attribution popover when its vehicles show |
...
- **Keyless forward search (Photon fallback).** With no Google key — or when Google declines (an unenabled Geocoding API answers HTTP 200 with `REQUEST_DENIED`) — place search falls through to Photon (`src/keylessGeocoder.js`): no key, komoot's free public instance under fair use, OSM data under ODbL. Photon normalises into the Google place shape (types, `{southwest, northeast}` bounds from its `[west, north, east, south]` extent, canonical feature name), biased by view-centre proximity that decides which match wins without changing what counts as a match. Answered misses are memoised; outages are never cached as misses. First use registers a "Keyless place search by Photon" credit in the Data attribution popover.
...
- **Transit (GTFS-Realtime).** `/api/transit/vehicles/<id>` fetches only the feed URLs registered in `src/data/transitFeeds.js` — the browser names a registered id, never a URL. Feeds are https-only (redirects must stay on https), capped at 8 MB, decoded server-side with `pbf`, cached in memory for 15 s per feed with single-flight refresh, and served stale for up to 10 minutes during an outage; nothing is written to disk. Vehicle positions are operator-reported telemetry, typically seconds old on arrival and shown one poll interval behind so they glide — not evidence of where a vehicle is this second.
