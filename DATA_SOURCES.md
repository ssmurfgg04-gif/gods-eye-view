...
| **OpenStreetMap (Nominatim)** | Reverse-geocoded place label in the cockpit Local Info page + keyless forward-search fallback ("fly to X" with no Google key) | ODbL 1.0 + Nominatim usage policy | "© OpenStreetMap contributors" |
...
- **Keyless forward search (Nominatim fallback).** With no Google key — or when Google misses/quota-fails — place search falls through to Nominatim `/search` (`src/geocodeFallback.js`): no key, ODbL data, throttled to ≤1 req/s with de-duplicated in-flight requests and a bounded cache per the Nominatim usage policy. Fallback hits are unbiased global point results (no viewport bias, no place viewport — the camera flies to the point at default range) and register a "Search fallback © OpenStreetMap contributors via Nominatim" credit in the Data attribution popover the first time they fire.
