...
import { GEV_REALTIME_TOOLS } from './src/voice/realtimeTools.js';
...
import { installLocalVoiceMiddleware } from './server/localVoice.js';
        ...
        try {
          ...
          try {
            upstream = await fetch(upstreamUrl.toString(), {
              ...
              signal: controller.signal,
              // PR #180: refuse redirects outright. fetch()'s default
              // redirect: 'follow' would silently hop OFF the allowlisted
              // host to any Location the upstream (or an attacker
              // controlling it) names — an allowlist bypass with
              // cache-poisoning potential. A GBFS feed must answer at its
              // canonical URL or not at all.
              redirect: 'error',
            });
          } catch (redirectError) {
            clearTimeout(timeoutId);
          ...
          // Limit response size to prevent memory exhaustion from malicious
          // upstream (PR #180: the cap is enforced WHILE STREAMING, not only
          // after the body has been fully buffered — an upstream that omits
          // content-length could otherwise stream unbounded bytes into
          // memory before the post-hoc byte check runs).
          ...
          if (upstream.body && typeof upstream.body.getReader === 'function') {
            ...
            for (;;) {
              ...
              if (received > GBFS_MAX_BODY_BYTES) {
            ...
            // Minimal fetch shims without a streaming body: fall back to
            // buffered text with the post-hoc byte check (still bounded by
            // the content-length pre-check above).
 ...
 */
/**
 * Voice-control instructions shared by every voice provider (OpenAI Realtime
 * and the local provider in server/localVoice.js). Extracted so a second
 * provider cannot drift from the first.
 */
...
export function openAiRealtimeProxy() {
  function install(middlewares) {
    ...
    // Local voice provider (Ollama / Anthropic) — same tools, same
    // instructions, no OpenAI key. See server/localVoice.js.
    installLocalVoiceMiddleware(middlewares, { tools: GEV_REALTIME_TOOLS, instructions: GEV_VOICE_INSTRUCTIONS });
...
/**
 * LAN-share session token gate (SECURITY hardening).
 *
 * When the dev server binds to 0.0.0.0/:: ("share on LAN" mode), every
 * /api/* proxy upstream consumes the HOST's provider keys and quotas.
 * Previously ALL LAN traffic was trusted once sharing was on: any device
 * on the network could drain the host's OpenSky/OpenAI/FIRMS/TomTom
 * credentials with no authentication. This gate fixes that:
 *
 * - A per-boot random session token is generated when LAN binding is active.
 * - Loopback requests (the host's own browser) pass unconditionally.
 * - Non-loopback /api/* requests must present the token as the
 *   `x-gev-token` header or `?gev_token=` query parameter, else 403.
 * - GET /api/lan-token is loopback-ONLY: the host's browser reads the token
 *   once (src/lanToken.js) and can hand the tokenized URL to guests.
 * - The static app shell stays reachable without a token — only the
 *   key-consuming proxy surface is gated.
 *
 * Loopback-only dev (`HOST` unset or localhost) is completely unchanged.
 */
function lanShareTokenGate(env) {
  const lanBound = env.HOST === '0.0.0.0' || env.HOST === '::';
  const sessionToken = lanBound ? (globalThis.crypto?.randomUUID?.() || require('node:crypto').randomUUID()) : null;
  const LOOPBACK_ADDRESSES = new Set(['127.0.0.1', '::1', '::ffff:127.0.0.1']);
  const isLoopback = (req) => LOOPBACK_ADDRESSES.has(req?.socket?.remoteAddress || '');
  const presentedToken = (req) => {
    const header = req.headers?.['x-gev-token'];
    if (typeof header === 'string' && header.trim()) return header.trim();
    try {
      const url = new URL(req.url || '/', 'http://localhost');
      const query = url.searchParams.get('gev_token');
  ...
  const denied = (res) => {
    res.writeHead(403, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
    res.end(JSON.stringify({ error: 'LAN share token required for API access' }));
  };
  return {
    name: 'lan-share-token-gate',
    configureServer(server) {
      if (!sessionToken) return;
      console.info(`[LAN Share] API gate active. Session token: ${sessionToken}`);
      console.info('[LAN Share] Share URLs for guests: append ?gev_token=<token> (the host browser stores it automatically).');
      server.middlewares.use((req, res, next) => {
        ...
        if (path.startsWith('/api/lan-token')) {
          if (isLoopback(req)) {
            res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
        ...
        if (isLoopback(req) || presentedToken(req) === sessionToken) {
          // Strip the token from the URL so downstream proxies and logs
          // never see it; the header (if any) is forwarded harmlessly.
          const queryIndex = path.indexOf('?');
          if (queryIndex >= 0 && path.includes('gev_token=')) {
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      ...
      rollupOptions: {
        output: {
          // Chunk the app's own dependency families so the long-lived engine
          // code caches independently of the frequently-changing app code
          // (perf: repeat visitors re-download only what changed).
          // - geo: the map/vector math family pulled in by satellite
          //   tracking, MGRS readouts, and vector-tile traffic decoding.
          // - detection: the Canvas2D contact overlay family (detection,
          //   label arbitration, drawing) shared by multiple layer chunks.
          // Entry stays lazy: layer implementations are dynamically imported
          // via src/data/layerManifest.js and voice/annotations/scenes via
          // main.js's deferred subsystem loader, so they already emit as
          // separate chunks WITHOUT manual chunks. Do not add those here —
          // manualChunks would hoist them back into the eager graph.
          manualChunks(id) {
            if (id.includes('node_modules')) {
              // NOTE: egm96-universal is deliberately NOT here — it is only
              // ever dynamically imported (src/data/geoid.js), and any
              // manual chunk assignment would eagerly attach its ~2.7 MB
              // grid to this chunk's static importers.
              if (id.includes('satellite.js') || id.includes('/pbf') || id.includes('vector-tile') || id.includes('mgrs')) {
                return 'geo';
              }
              return undefined;
            }
