...
- **Same-site gate on paid-or-writable endpoints.** `/api/realtime/token`, `/api/openai/hud-summary`, `/api/google/nearby-places`, and `/api/realtime/debug-log` refuse cross-site requests: reverse-proxy signal headers, a `Sec-Fetch-Site` other than `same-origin`/`none`, and a foreign or opaque `Origin` each earn a 403 (`src/localRequestGate.mjs`, pure and unit-tested). The gate deliberately does not require a loopback socket, an `Origin`, or JSON, so the `HOST=0.0.0.0` LAN opt-in and headerless Node QA harnesses keep working. A hostile page can therefore neither mint Realtime tokens, spend provider quota, nor append bytes to the debug log from another site.
- **Full Content-Security-Policy on dev and preview.** `script-src` is `'self' 'unsafe-eval'` — Knockout (bundled in `@cesium/widgets`) resolves the global object with `eval` at load time and the Cesium widget never initializes without it; inline scripts and foreign script origins stay blocked. `connect-src` intentionally stays broad (the app calls dozens of provider hosts directly; that boundary lives in the proxies), while `frame-ancestors 'none'`, `object-src 'none'`, and `base-uri 'self'` hold.
...
- **Local-only by default.** `./scripts/dev-fresh.sh` (and the Vite config itself) bind to `localhost`, so only your machine can reach the server — and only local names are accepted (`allowedHosts` is never `true`, which also blunts DNS-rebinding tricks; wide-open binds additionally admit this machine's LAN IPv4s plus `VITE_ALLOWED_HOSTS` extras, never arbitrary Host headers).
